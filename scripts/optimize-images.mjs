...
function getOldOutputs(image) {
  if (Array.isArray(image.variants)) {
    return image.variants.map((variant) => variant.output);
  }

  return typeof image.output === "string" ? [image.output] : [];
}
...
async function optimizeProfile(profileName) {
  ...
  const activeOutputs = new Set(
    images.flatMap((image) => image.variants.map((variant) => variant.output)),
  );
  let removedCount = 0;

  for (const previous of previousManifest.images) {
    for (const oldOutput of getOldOutputs(previous)) {
      if (activeOutputs.has(oldOutput) || !oldOutput.startsWith(publicPrefix)) continue;

      const stalePath = path.join(
        outputDirectory,
        oldOutput.slice(publicPrefix.length),
      );
      assertInside(outputDirectory, stalePath);
  ...
  console.log(
    `[images:${profileName}] ${images.length} source(s): ${optimizedCount} optimized, ` +
      `${skippedCount} unchanged, ${removedCount} stale output(s) removed, ` +
